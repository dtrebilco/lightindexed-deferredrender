...
/*	for (uint i = 0; i < MAX_SAMPLERSTATE; i++){
	...
	if (flags & RESET_DS) selectedDepthState = DS_NONE;
	if (flags & RESET_BS) selectedBlendState = BS_NONE;
...
/*	for (uint i = 0; i < MAX_SAMPLERSTATE; i++){
	...
	if (selectedDepthState != DONTCARE) changeDepthState(selectedDepthState);
	if (selectedBlendState != DONTCARE) changeBlendState(selectedBlendState);
...
TextureID Renderer::addCubemap(const char **fileNames, const bool useMipMaps, const SamplerStateID samplerState, uint flags){
	Image img;
	if (img.loadSlicedImage(fileNames, 0)){
		if (img.getFormat() == FORMAT_RGBE8) img.unpackImage();
		if (useMipMaps && img.getMipMapCount() <= 1) img.createMipMaps();
		return addTexture(img, samplerState, flags);
	} else {
