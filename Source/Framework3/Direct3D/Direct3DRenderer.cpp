...
void Direct3DRenderer::resetToDefaults(){
	...
	currentSrcFactor = ONE;
	currentDstFactor = ZERO;
...
/*void setupFilter(Texture &tex, Filter filter, const uint flags){
	tex.filter.minFilter = hasAniso(filter)? D3DTEXF_ANISOTROPIC : (filter != NEAREST)? D3DTEXF_LINEAR : D3DTEXF_POINT;
	tex.filter.magFilter = hasAniso(filter)? D3DTEXF_ANISOTROPIC : (filter != NEAREST)? D3DTEXF_LINEAR : D3DTEXF_POINT;
	tex.filter.mipFilter = (filter == TRILINEAR || filter == TRILINEAR_ANISO)? D3DTEXF_LINEAR : hasMipmaps(filter)? D3DTEXF_POINT : D3DTEXF_NONE;
	tex.filter.wrapS = (flags & CLAMP_S)? D3DTADDRESS_CLAMP : D3DTADDRESS_WRAP;
	tex.filter.wrapT = (flags & CLAMP_T)? D3DTADDRESS_CLAMP : D3DTADDRESS_WRAP;
	tex.filter.wrapR = (flags & CLAMP_R)? D3DTADDRESS_CLAMP : D3DTADDRESS_WRAP;
}*/
...
TextureID Direct3DRenderer::addRenderTarget(const int width, const int height, const int depth, const int arraySize, const FORMAT format, const int msaaSamples, const SamplerStateID samplerState, uint flags){
...
bool Direct3DRenderer::resizeRenderTarget(const TextureID renderTarget, const int width, const int height, const int depth, const int arraySize){
...
SamplerStateID Direct3DRenderer::addSamplerState(const Filter filter, const AddressMode s, const AddressMode t, const AddressMode r, const float lod){
	...
	samplerState.wrapS = (s == WRAP)? D3DTADDRESS_WRAP : D3DTADDRESS_CLAMP;
	samplerState.wrapT = (t == WRAP)? D3DTADDRESS_WRAP : D3DTADDRESS_CLAMP;
	samplerState.wrapR = (r == WRAP)? D3DTADDRESS_WRAP : D3DTADDRESS_CLAMP;
	samplerState.lod = lod;

	return samplerStates.add(samplerState);
}

BlendStateID Direct3DRenderer::addBlendState(const int srcFactor, const int destFactor, const int blendMode, const int mask){
	BlendState blendState;

	blendState.srcFactor = srcFactor;
	blendState.dstFactor = destFactor;
	blendState.blendMode = blendMode;
	blendState.mask = mask;
	blendState.blendEnable = (srcFactor != ONE || destFactor != ZERO);
...
void Direct3DRenderer::changeRenderTargets(const TextureID *colorRTs, const uint nRenderTargets, const TextureID depthRT, const int *faces){
...
void Direct3DRenderer::changeBlendState(const BlendStateID blendState){
	if (blendState != currentBlendState){
		if (blendState == BS_NONE || !blendStates[blendState].blendEnable){
			...
			if (blendStates[blendState].blendEnable){
				...
				if (blendStates[blendState].srcFactor != currentSrcFactor)
					dev->SetRenderState(D3DRS_SRCBLEND,  currentSrcFactor = blendStates[blendState].srcFactor);
				if (blendStates[blendState].dstFactor != currentDstFactor){
					dev->SetRenderState(D3DRS_DESTBLEND, currentDstFactor = blendStates[blendState].dstFactor);
				}
				if (blendStates[blendState].blendMode != currentBlendMode){
					dev->SetRenderState(D3DRS_BLENDOP, currentBlendMode = blendStates[blendState].blendMode);
				}
			}
		}
...
void Direct3DRenderer::changeDepthState(const DepthStateID depthState){
...
void Direct3DRenderer::clear(const bool clearColor, const bool clearDepth, const float *color, const float depth){
	...
	dev->Clear(0, NULL, flags, clearCol, depth, 0);
}
