...
unsigned char *Image::create(const FORMAT fmt, const int w, const int h, const int d, const int mipMapCount){
    format = fmt;
    width  = w;
	height = h;
	depth  = d;
	...
	return (pixels = new unsigned char[getMipMappedSize(0, nMipMaps)]);
}
...
bool Image::loadSlicedImage(const char **fileNames, const int nImages, uint flags){
	int maxImage = nImages? nImages : 6;

	Image *images = new Image[maxImage];
	...
	ubyte *dest = create(images[0].format, images[0].width, images[0].height, nImages, nMipMaps);

	for (uint level = 0; level < nMipMaps; level++){
		int size = images[0].getMipMappedSize(level, 1);
		for (int i = 0; i < maxImage; i++){
			memcpy(dest, images[i].getPixels(level), size);
			dest += size;
		}
	}
...
bool Image::createMipMaps(const int mipMaps){
	...
	if (nMipMaps != actualMipMaps){
		pixels = (ubyte *) realloc(pixels, getMipMappedSize(0, actualMipMaps));
	...
	ubyte *src = getPixels(0);
	ubyte *dst = getPixels(1);
...
bool Image::convert(const FORMAT newFormat){
	...
	if (format == FORMAT_RGBE8 && (newFormat == FORMAT_RGB32F || newFormat == FORMAT_RGBA32F)){
		newPixels = new ubyte[getMipMappedSize(0, nMipMaps, newFormat)];
		...
		ubyte *src = pixels;
		ubyte *dest = newPixels = new ubyte[getMipMappedSize(0, nMipMaps, newFormat)];
				...
				if (isFloatFormat(newFormat)){
					if (newFormat <= FORMAT_RGBA32F){
						...
						if (newFormat == FORMAT_RGBE8){
							*(uint32 *) dest = rgbToRGBE8((vec3 &) rgba);
						} else {
							*(uint32 *) dest = rgbToRGB9E5((vec3 &) rgba);
						}
					}
	...
	for (int mipMap = 0; mipMap < nMipMaps; mipMap++){
		...
		for (int y = 0; y < h; y++){
			for (int x = 0; x < w; x++){
				...
				float rX = xFactor * (sX * invLen + 1.0f);
				float rY = yFactor * (sY * invLen + 1.0f);
				float rZ = zFactor * (sZ * invLen + 1.0f);
...
bool Image::toGrayScale(){
	...
	uint len = getPixelCount(0, nMipMaps);
	uint size = len;
...
bool Image::getRange(float &min, float &max){
	...
	int nElements = getPixelCount(0, nMipMaps) * getChannelCount(format);
